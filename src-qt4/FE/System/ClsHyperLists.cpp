...
list<pair<int, double> > ListIndexTriples::getByFirst(int i){
    list<pair<int, double> > lst;
    list<IndexTriple>::iterator it;
...
list<pair<int, double> > ListIndexTriples::getBySecond(int i){
    list<pair<int, double> > lst;
    list<IndexTriple>::iterator it;
    for(it=this->begin(); it!=this->end(); it++){
	if((*it).second == i){
...
vector<pair<int, int> > ListIndexQuadruples::getDelayByFirst(int i){
    vector<pair<int, int> > lst;
    vector<IndexQuadruple>::iterator it;
    for(it=this->begin(); it!=this->end(); it++){
	if((*it).first == i){
	    pair<int, int> pairTemp((*it).second, (*it).delay);
	    lst.push_back(pairTemp);
	}
    }
    return lst;
...
vector<pair<int, int> > ListIndexQuadruples::getDelayBySecond(int i){
    vector<pair<int, int> > lst;
    vector<IndexQuadruple>::iterator it;
    for(it=this->begin(); it!=this->end(); it++){
	if((*it).second == i){
	    pair<int, int> pairTemp((*it).first, (*it).delay);
	    lst.push_back(pairTemp);
	}
    }
    return lst;
...
vector<Indices> ListIndexQuadruples::getIndicesByFirst(int i){
    ...
    // according to the list.  All list users should adhere to this
    // convention.
...
list<int> ListIndexQuadruples::getDelays(){
    ...
    // 22.10.2003 [jmb] List needs to be sorted before calling unique.
    lst.sort();
    lst.unique();
    return lst;
}
