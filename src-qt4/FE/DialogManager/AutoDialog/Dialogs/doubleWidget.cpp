...
iqrfe::ClsDoubleWidget::ClsDoubleWidget(ClsDoubleParameter &_parameter, 
					...
					const char *_pcName) : ClsParameterWidget(_parameter,
										  false, 
										  _pqwgtParent,
										  _pcName),
    ...
    QHBoxLayout *pqlayLayout = new QHBoxLayout(this /*, 0, SMALL_VSPACE, QString(_pcName)+" layout"*/);
    ...
    connect(pValueWidget, SIGNAL(valueChanged(double)), this, SLOT(setValueChanged()));
    // Emit each value change for setting the min or max values in
    // other widgets.
    connect(pValueWidget, SIGNAL(valueChanged(double)), this, SIGNAL(valueChanged(double)));
...
iqrfe::ClsDoubleWidget::~ClsDoubleWidget() {
	...
	cout << "ClsDoubleWidget::~ClsDoubleWidget" << endl;
    }
}
 ...
 */
double iqrfe::ClsDoubleWidget::getValue() const {
 ...
 * library string.
 */
string iqrfe::ClsDoubleWidget::getValueAsString() const {
 ...
 */
void iqrfe::ClsDoubleWidget::setValue(double _dValue) {
 ...
 * string.
 */
void iqrfe::ClsDoubleWidget::setValueFromString(string _strValue) {
 ...
 */
void iqrfe::ClsDoubleWidget::connectWidget(const ParameterWidgetMap &_mapWidgets) {
    if (bDebugDoubleWidget) {
	cout << "ClsDoubleWidget::connectWidget" << endl;
    }
 ...
 */
void iqrfe::ClsDoubleWidget::showEvent(QShowEvent *_e) {
    if (!_e->spontaneous()) {
	if (bDebugDoubleWidget) {
...
void iqrfe::ClsDoubleWidget::setValueChanged() {
    bValueChanged = true;
 ...
 */
void iqrfe::ClsDoubleWidget::connectToMinSetter(const ParameterWidgetMap &_mapWidgets) {
    if (bDebugDoubleWidget) {
	cout << "ClsDoubleWidget::ConnectToMinSetter" << endl;
    }
	...
	if (bDebugDoubleWidget) {
	    cout << "ClsDoubleWidget::ConnectToMinSetter: found min setter" << endl;
	}	
	
	// If found, connect min setter's valueChanged(double) SIGNAL to the
	// setMinimum(double) SLOT of this widget's spin box.
	connect(pMinSetter, SIGNAL(valueChanged(double)), pValueWidget, SLOT(setMinimum(double)));
    }
}
 ...
 * @param &_mapWidgets Map of parameter widgets to search.  
 */
void iqrfe::ClsDoubleWidget::connectToMaxSetter(const ParameterWidgetMap &_mapWidgets) {
    if (bDebugDoubleWidget) {
	cout << "ClsDoubleWidget::ConnectToMaxSetter" << endl;
    }
    ...
    if (bDebugDoubleWidget) {
	cout << "ClsDoubleWidget::ConnectToMaxSetter: searching for widget " << getMaxSetter() << endl;
    }

    ClsParameterWidget *pMaxSetter;
    if ((pMaxSetter = findWidget(_mapWidgets, getMaxSetter())) != 0) {
	if (bDebugDoubleWidget) {
	    cout << "ClsDoubleWidget::ConnectToMaxSetter: found max setter" << endl;
	}	
	
	// If found, connect max setter's valueChanged(double) SIGNAL
	// to the setMaximum(double) SLOT of this widget's spin box.
	connect(pMaxSetter, SIGNAL(valueChanged(double)), pValueWidget, SLOT(setMaximum(double)));
    }
}
