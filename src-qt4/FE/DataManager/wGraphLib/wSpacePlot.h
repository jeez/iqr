...
public:
  ...
  QVector<SpaceCell> &getCellData() { return cellData; }
  ...
  /*! Returns a reference to a list of indices of the currently
    selected cells.*/
  QVector<int> &getSelecIndexList();

  /*! Selects the cells with indices given in \em list. */
  void setSelecIndexList(QVector<int > &list);
...
protected:
  ...
  QVector<int> selecIndexList;
  QVector<SpaceCell> cellData;

  bool selecting, dragEnabled;
  bool drawGrid, autoScale, multiSelect;

  QRubberBand *qrbSelection;
