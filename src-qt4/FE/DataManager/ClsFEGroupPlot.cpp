...
//--    connect( wSpacePlot, SIGNAL(mouseEvent(QEvent::Type, QPoint, int, int)), this, SLOT(slotPlotMouseClick(QEvent::Type, QPoint, int, int)) );
...
//--
//--    if(iButton == 2){
//--	Q3PopupMenu *menuBrowser = new Q3PopupMenu( this );
...
//--    case MENU_TIME_PLOT:
//--	strParamList = dynamic_cast<ClsQNeuronStateVariableDisplay*>(clsQBaseStateVariableDisplay)->getSelectedStatesAsString();
//--	qvectorSelected =  wSpacePlot->getSelecIndexList ();
//--	strRange = wSpacePlotSelected2Range(qvectorSelected);
//--	clsFEDataManager->DataClientCreate(ClsFEDataClient::CLIENT_TIMEPLOT, clsQBaseStateVariableDisplay->getItemID(), strParamList, strRange);
//--	break;
//--    default:
//--	break;
//--    }
...
string ClsFEGroupPlot::wSpacePlotSelected2Range(QVector<int> &qvectorSelected){
	...
	for(unsigned int ii = 0; ii< qvectorSelected.size(); ii++){
	    int iy = int(qvectorSelected[ii] / iGroupWidth) + 1;
	    int ix = qvectorSelected[ii] - int(qvectorSelected[ii] / iGroupWidth) * iGroupWidth + 1;
...
//		cout << __FILE__ << ": ix, iy: " << ix << ", " << iy << endl;
	...
	for(unsigned int ii = 0; ii< qvectorSelected.size(); ii++){
	    strRange = iqrUtils::int2string(qvectorSelected[ii]) + ";"  +  strRange ;
	}
    }
    return strRange;
}
