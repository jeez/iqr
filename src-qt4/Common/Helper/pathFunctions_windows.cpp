/**
 * WINDOWS implementation of path functions needed to load libraries.
 * SINCE WE USE MINGW, THESE FUNCTIONS ARE _ALMOST_ IDENTICAL TO THE UNIX ONES
 * SPECIFICALLY, WE USE THE SAME DIRECTORY SEPARATORS....
 *
...
char iqrcommon::getDirectorySeparator() {
  if (bDebugPathFunctions) {
    cout << "iqrcommon::getDirectorySeparator()" << endl;
  }
  return '/';
}

char iqrcommon::getExtensionSeparator() {
  if (bDebugPathFunctions) {
    cout << "iqrcommon::getExtensionSeparator()" << endl;
  }

  return '.';
}

string iqrcommon::getLibraryExtension() {
  if (bDebugPathFunctions) {
    cout << "iqrcommon::getLibraryExtension()" << endl;
  }

  return string(1, getExtensionSeparator()) + "dll";
}
//
//////////////////////////////////
...
iqrcommon::fileNameHasExtension(string _strFileName, string _strExtension) {
  if (bDebugPathFunctions) {
    cout << "fileNameHasExtension(string _strFileName, string _strExtension)" << endl;
  }
 ...
 */
string iqrcommon::getFileBaseName(string _strFileName) {
  if (bDebugPathFunctions) {
    cout << "iqrcommon::getFileBaseName(string _strFileName)" << endl;
  }
 ...
 * specify ".txt").
 */
list<string> iqrcommon::getListFiles(string _strDirectory, string _strExtension) {
  if (bDebugPathFunctions) {
    cout << "iqrcommon::getListFiles(string _strDirectory, string _strExtension)" << endl;
  }

  list<string> fileList;

  // WARNING: ON WINDOWS FOR stat THE DIRECTORY CANNOT END WITH A SEPARATOR!
  string directoryName;
  if (_strDirectory[_strDirectory.length()-1] == getDirectorySeparator()) {
    int ii = _strDirectory.length()-1;
    directoryName = _strDirectory.substr(0, ii);
  } else {
    directoryName = _strDirectory;
  }


  // Check the path name really describes a directory
  //    directoryName ="c:/home/ulysses/iqr/lib/Neurons";
  // directoryName ="C:/Dokumente und Einstellungen/Ulysses Bernardet/iqr/lib/Neurons";
  struct stat directoryStatus;
  if (stat(directoryName.c_str(), &directoryStatus) < 0) {
    // Error getting status of directory
    cerr << "ERROR: Getting status of directory: " << directoryName << endl;
    return fileList;
  }

  if (S_ISDIR(directoryStatus.st_mode) == 0) {
    // Specified path is not a directory
    cerr << "ERROR: Specified path is not a directory" << endl;
    return fileList;
  }
  ...
  if (pDir == NULL) {
    cerr << "ERROR: Couldn't open directory: " << directoryName << endl;
    return fileList;
  }
  ...
  while ((pDirEntry = readdir(pDir)) != NULL) {
    entryName = addDirectorySeparator(directoryName) + pDirEntry->d_name;
    if (bDebugPathFunctions) {
      cout << "entryName: " << entryName << endl;
    }
 ...
 */
string iqrcommon::addDirectorySeparator(string _strPath) {
  if (bDebugPathFunctions) {
    cout << "iqrcommon::addDirectorySeparator(string _strPath)" << endl;
  }
 ...
 */
list<string> iqrcommon::getListLibraries(string _strDirectory) {
  if (bDebugPathFunctions) {
    cout << "iqrcommon::getListLibraries(string _strDirectory)" << endl;
  }
 ...
 * @param _strFileName File name.
 */
string iqrcommon::getPathToFile(string _strDirectory, string _strFileName) {
  if (bDebugPathFunctions) {
    cout << "iqrcommon::getPathToFile(string _strDirectory, string _strFileName)" << endl;
  }
