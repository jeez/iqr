...
*/
    ...
    aiProximitySensorPos[0] = 270;
    aiProximitySensorPos[1] = 310;
    aiProximitySensorPos[2] = 0;
    aiProximitySensorPos[3] = 90;   
    aiProximitySensorPos[4] = 90;
    aiProximitySensorPos[5] = 180;
    aiProximitySensorPos[6] = 230; 
    aiProximitySensorPos[7] = 270;
...
//    cout << "ClsEPuckDisp::DrawStatus()" << endl;
...
//    cout << "ClsEPuckDisp::DrawSoma(QPainter *_Painter, int iWhiskersWidth)" << endl;
    int ii;
...
// Proximity and Ambient
    ...
    for ( fAngle = 290; fAngle <= 360; fAngle += 35){
	_Painter->setPen( QPen(  colLines, 1, Qt::SolidLine ) );
	...
	iXPos = (int)(cos( fAngle * PI / 180.0) * ((double)iSomaDiameter / 2.0  + iAmbientSensorDiameter / 2 )) + iXCenter;
	iYPos = (int)(sin( fAngle * PI / 180.0) * ((double)iSomaDiameter / 2.0  + iAmbientSensorDiameter / 2 )) + iYCenter;

	iXPosText = (int)(cos( fAngle * PI / 180.0) * ((double)iSomaDiameter / 2.0  - iAmbientSensorDiameter )) + iXCenter;
	iYPosText = (int)(sin( fAngle * PI / 180.0) * ((double)iSomaDiameter / 2.0  - iAmbientSensorDiameter )) + iYCenter;
	...
	QString qstr;
	qstr.setNum(iCountSensor);
	_Painter->drawText ( iXPosText, iYPosText, qstr );
    ...
    for ( fAngle = 180; fAngle <= 250; fAngle += 35){
	_Painter->setPen( QPen( colLines, 1, Qt::SolidLine ) );
	iAmbientColor = (int)(vaAmbient[iCountSensor]*254.0);
