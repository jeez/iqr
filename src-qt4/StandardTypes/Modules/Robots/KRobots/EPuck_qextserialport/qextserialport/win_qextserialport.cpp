...
bool Win_QextSerialPort::open(OpenMode mode) {
    ...
    TTY_WARNING("not open");

//new        Win_Handle=CreateFileA(port.toAscii(), 
//new			       GENERIC_READ|GENERIC_WRITE,
//new			       FILE_SHARE_READ|FILE_SHARE_WRITE, 
//new			       NULL, 
//new			       OPEN_EXISTING, 
//new			       0, 
//new			       NULL
//new			       );
    ...
    QString qstrPort(port);
    qstrPort = "\\\\.\\" + qstrPort; /* don't ask why...*/


	Win_Handle = CreateFile(qstrPort.toAscii(),
	   ...
	   TTY_WARNING("Win_Handle == INVALID_HANDLE_VALUE");
	}
    }
    UNLOCK_MUTEX();
    return isOpen();
