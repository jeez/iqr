...
<xsl:text>         </xsl:text><xsl:value-of select="@id"/>[label="<xsl:value-of select="@name"/>\n[<xsl:value-of select="@id"/>]",fontname=Helvetica];
<!-- --></xsl:for-each><!--
-->    }
</xsl:for-each>
...
<xsl:text>         </xsl:text><xsl:value-of select="@source"/>-><xsl:value-of select="@target"/>[label="<xsl:value-of select="@id"/> (<xsl:value-of select="@source"/>-><xsl:value-of select="@target"/>)",fontname=Helvetica,fontsize=10,minlen=3];
</xsl:for-each>
