...
void ClsEPuckDisp::ClearImg(){
    _buffer->fill(black );
    bitBlt (this, 0, 0, _buffer);
}

void ClsEPuckDisp::PassBack( int iPassBack ){
    if( iPassBack == RESET){
	fYMax = 0;
	fYMin = 1000000;
	_buffer->fill(black );
...
void ClsEPuckDisp::resizeEvent( QResizeEvent* event) {
...
void ClsEPuckDisp::customEvent( QCustomEvent* e ) {
