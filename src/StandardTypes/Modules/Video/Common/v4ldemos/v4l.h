...
// Mask and type to name conversion
    ...
    extern char *V4LChannelTypeName(int channelType); // V4LChannel.type
    extern char *V4LTunerModeName(int tunerMode);     // V4LTuner.mode
    extern char *V4LFormatName(int format);           // V4LFormat.format, V4LPicture.palette
...
//---------------------------------------------------------------------------------
    ...
    extern int V4LGetCaps(int fd, V4LCaps *v4lCaps);

    extern int V4LSetSource(int fd, int channel, int norm);
    extern int V4LGetSource(int fd, int channel, V4LChannel *v4lChannel);

    extern int _V4LSetPicture(int fd, V4LPicture *v4lPicture);
    extern int _V4LGetPicture(int fd, V4LPicture *v4lPicture);

    extern int V4LSetBrightness(int fd, int brightness); // 0-65535 -> +/-50% luma offset
    extern int V4LSetContrast(int fd, int contrast);     // 0-65535 -> 0-200% luma gain
    extern int V4LSetSaturation(int fd, int saturation); // 0-65535 -> 0-200% chroma gain
    extern int V4LSetHue(int fd, int hue);               // 0-65535 -> +/-90o hue phase

// Overlay
    extern int V4LOSetFramebuffer(int fd, V4LFramebuffer *v4lFramebuffer); // depth => format
    extern int V4LOGetFramebuffer(int fd, V4LFramebuffer *v4lFramebuffer);
    extern int V4LOSetWindow(int fd, V4LWindow *v4lWindow);
    extern int V4LOGetWindow(int fd, V4LWindow *v4lWindow);
    extern int V4LOCapture(int fd, int on); // 0=off, 1=on

// Memory
    extern int V4LMGetMMInfo(int fd, V4LMMInfo *v4lMMInfo);
    extern void V4LMSetFormat(V4LFormat *v4lFormat);
    extern void V4LMGetFormat(V4LFormat *v4lFormat);
    extern int V4LMCapture(int fd, int frame);
    extern int V4LMSync(int fd, int frame);

// TV
    extern int V4LSetTuner(int fd, V4LTuner *v4lTuner);
    extern int V4LGetTuner(int fd, V4LTuner *v4lTuner);
    extern int V4LSetTunerFreq(int fd, ulong tunerFreq);
    extern int V4LGetTunerFreq(int fd, ulong *tunerFreq);
    extern int V4LSetAudio(int fd, V4LAudio *v4lAudio);
    extern int V4LGetAudio(int fd, V4LAudio *v4lAudio);
