...
vector<tiPoint> ClsBaseTopology::getCellsForArborizationRect(double fXCenter, double fYCenter, ClsBaseArborization *clsBaseArborization){
#ifdef DEBUG_CLSBASETOPOLOGY
    ...
    vector<tiPoint> lstPointsLocalProbAdjusted;
	...
	for(int iY=1;iY<=nrCellsVertical();iY++){
		...
		lstPointsLocal.push_back(pointTemp);
	    }
	}
    }
...
vector<tiPoint> ClsBaseTopology::AndLists(const vector<tiPoint>& lst0, const vector<tiPoint> &lst1) {
#ifdef DEBUG_CLSBASETOPOLOGY
    cout << "ClsBaseTopology::AndLists(const vector<tiPoint>& lst0, const vector<tiPoint> &lst1)" << endl;
#endif
    vector<tiPoint> lstReturn;

    vector<tiPoint>::const_iterator it0;
...
vector<tiPoint> ClsBaseTopology::XOrLists(const vector<tiPoint> &lst0, const vector<tiPoint> &lst1) {
#ifdef DEBUG_CLSBASETOPOLOGY
    cout << "ClsBaseTopology::XOrLists(const vector<tiPoint> &lst0, const vector<tiPoint> &lst1)" << endl;
#endif
    vector<tiPoint> lstReturn;
