 ...
 ** $Header: /home/ulysses/Code/iqr421_Redesign/FE/SystemFile/RCS/ClsFESerializer.cpp,v 1.4 2005/03/18 23:01:23 ulysses Exp ulysses $
 **
...
string ClsFESerializer::getProcessesAsXML(list<string> lstIDs) {
    ...
    DOMDocument *ddocClipboard = impl->createDocument(0, XMLString::transcode(ClsTagLibrary::iqrEntityTag()), NULL);
...
//	theSerializer->writeNode(myFormTarget, *delemProcess);
...
int ClsFESerializer::serializeToAER(string strExportFilename){
    ...
    DOMDocument *ddocAERSystem = impl->createDocument(0, XMLString::transcode(ClsTagLibrary::AERSystemTag()), dtd);
...
list<DOMElement*> ClsFESerializer::getAERConnections(DOMDocument *ddocRoot){
    if (bDebugSystemFileWriter) {
...
//					 XMLString::transcode(strConnectionKey.c_str()));
...
DOMElement *ClsFESerializer::addFESystem(DOMDocument *ddocRoot, ClsFESystem* clsFESystem) {
...
DOMElement *ClsFESerializer::addFEProcess(DOMDocument *ddocRoot, ClsFEProcess* clsFEProcess) {
...
//			       XMLString::transcode(clsFEProcess->getHostname().c_str()));
			       XMLString::transcode(""));

    delemProcess->setAttribute(XMLString::transcode(ClsTagLibrary::PortTag()), 
//			       XMLString::transcode(clsFEProcess->getPortAsString().c_str()));
...
DOMElement* ClsFESerializer::addFEGroup(DOMDocument *ddocRoot, ClsFEGroup *clsFEGroup) {
    if (bDebugSystemFileWriter) {
    ...
    delemGroup->setAttribute(XMLString::transcode(ClsTagLibrary::NameTag()), 
			     XMLString::transcode(clsFEGroup->getGroupName().c_str()));
	...
	vector<tiPoint> lstPInt = (dynamic_cast<ClsTopologySparse*>(clsBaseTopology))->getList();
	
	vector<tiPoint>::iterator itLstPInt;
...
DOMElement *ClsFESerializer::addFEConnection(DOMDocument *ddocRoot, ClsFEConnection* clsFEConnection) {
    if (bDebugSystemFileWriter) {
	...
	*/
	...
	if(!strPatternType.compare(ClsTagLibrary::ConnectionPatternMapped()) || !strPatternType.compare(ClsTagLibrary::ConnectionPatternForeach())){
		...
		if(dynamic_cast<ClsRegion*>(clsBaseSubPopulationSource)){
		    ...
		    vector<tiPoint> lstPInt = clsList->getData();
		    vector<tiPoint>::iterator itLstPInt;
		    for(itLstPInt=lstPInt.begin();itLstPInt!=lstPInt.end();itLstPInt++){
			int iX = (*itLstPInt).first;
			int iY = (*itLstPInt).second;
	    ...
	    if(clsTuples!=NULL){
		list<pair<tListOfPoints, tListOfPoints> > lstTuples = clsTuples->getData();
		list<pair<tListOfPoints, tListOfPoints> >::iterator itLPLP;
		
		for(itLPLP=lstTuples.begin(); itLPLP!=lstTuples.end(); itLPLP++){
		    pair<tListOfPoints, tListOfPoints> pairLOP = *itLPLP;
		    ...
		    tListOfPoints tlopSource = pairLOP.first;
		    ...
		    tListOfPoints::iterator itLPSource;
		    ...
		    tListOfPoints tlopTarget = pairLOP.second;
		    ...
		    tListOfPoints::iterator itLPTarget;
...
DOMElement* ClsFESerializer::addParameterizedNode(DOMDocument *ddocRoot, string strNodeTag, string strNodeType, ParameterList parameterList){
...
void ClsFESerializer::addXRefHolders(DOMDocument *ddocRoot, DOMElement *delemParent, ClsItem* itemParent){
