...
//XXXXX     iGroupWidth = 0;
//XXXXX
//XXXXX     vector<tiPoint>::iterator it;
...
//--    lstPoints.sort(pointCompareSparse(iGroupWidth));
    sort(lstPoints.begin(), lstPoints.end(),pointCompareSparse(iGroupWidth));
...
void ClsTopologySparse::setList(vector<tiPoint> _lst) {
#ifdef DEBUG_CLSTOPOLOGYSPARSE
    cout << "ClsTopologySparse::setList(vector<tiPoint> _lst)" << endl;
#endif
    lstPoints = _lst;
    iGroupWidth = 0;
...
vector<tiPoint> ClsTopologySparse::getPointsForSubPopulation(ClsBaseSubPopulation *clsBaseSubPopulation){
#ifdef DEBUG_CLSTOPOLOGYSPARSE
	...
	vector<tiPoint> lstPopulationList = (dynamic_cast<ClsList*>(clsBaseSubPopulation))->getData();
	...
	vector<tiPoint>lstRegion = region2pointList(clsRegion);
...
vector<tiPoint> ClsTopologySparse::checkList(const vector<tiPoint>& lstIn) {
#ifdef DEBUG_CLSTOPOLOGYSPARSE
    cout << "ClsTopologySparse::checkList(const vector<tiPoint>& lstIn)" << endl;
#endif
...
//--    lst.sort(pointCompareSparse(iGroupWidth));
    sort(lst.begin(), lst.end(), pointCompareSparse(iGroupWidth));
    return lst;
}



vector<tiPoint> ClsTopologySparse::getPoints4Ellipse(double fXCenter, double fYCenter, double fWidth, double fHeight) {
#ifdef DEBUG_CLSTOPOLOGYSPARSE
    ...
    vector<tiPoint> lstReturn = AndLists(lst, lstPoints);
    return lstReturn;
}


vector<tiPoint> ClsTopologySparse::getPoints4Rect(double fXCenter, double fYCenter, double fWidth, double fHeight) {
#ifdef DEBUG_CLSTOPOLOGYSPARSE
...
vector<tiPoint> ClsTopologySparse::region2pointList(ClsRegion *clsRegion) {
#ifdef DEBUG_CLSTOPOLOGYSPARSE
...
bool ClsTopologySparse::isValidCell(const vector<tiPoint>& _lst, int _iX, int _iY) {
#ifdef DEBUG_CLSTOPOLOGYSPARSE
    cout << "ClsTopologySparse::isValidCell(const vector<tiPoint>& _lst, int _iX, int _iY)" << endl;
#endif
...
bool ClsTopologySparse::isValidIndex(const vector<tiPoint>& _lst, int /* _iWidth */, int iIndex) {
#ifdef DEBUG_CLSTOPOLOGYSPARSE
    cout << "ClsTopologySparse::isValidIndex(const vector<tiPoint>& _lst, int /* _iWidth */, int iIndex)" << endl;
#endif
...
//--    lstPoints.sort(pointCompareSparse(_iWidth));
    sort(lstPoints.begin(), lstPoints.end(), pointCompareSparse(_iWidth));
    tiPoint p;
    p.first = iX;
    p.second = iY;

    vector<tiPoint>::iterator pos;
...
list<int> ClsTopologySparse::posList2indexList(const vector<tiPoint>& lstIn) {
#ifdef DEBUG_CLSTOPOLOGYSPARSE
    cout << "ClsTopologySparse::posList2indexList(const vector<tiPoint>& lstIn)" << endl;
#endif
    list<int> lstReturn;
