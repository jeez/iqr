...
//	cerr << "ClsFEConnection::calculateArbPattern()::strConnectionSourceID.size()<=0 || strConnectionTargetID.size()<=0" << endl;
    ...
    VectorPointTriples listPointTriples;
...
//	    cout << "fAreaWidth, fAreaHeight: " << fAreaWidth << ", " <<  fAreaHeight << endl;
	    ...
	    tVectorOfPoints tVectorOfPointsTarget = clsBaseTopologyTarget->getPointsForSubPopulation(clsBaseSubPopulationTarget);
//	    cout << "getting list for target" << endl;
//	    cout << "tVectorOfPointsTarget.size(): " << tVectorOfPointsTarget.size() << endl;
	    tVectorOfPoints::iterator itLOPTarget;
//	    cout << "-----------------------" << endl;

	    int iXStart = (*tVectorOfPointsTarget.begin()).first;
	    int iYStart = (*tVectorOfPointsTarget.begin()).second;
	    for(itLOPTarget=tVectorOfPointsTarget.begin();itLOPTarget!=tVectorOfPointsTarget.end();itLOPTarget++){
...
//		cout << "fXCenterProjected, fYCenterProjected: " << fXCenterProjected << ", " << fYCenterProjected << endl;
		...
		if(bAll){
		    tVectorOfPoints tVectorOfPointsSource;
		    tVectorOfPointsSource = clsBaseTopologySource->getPoints4Rect( fXCenterProjected, fYCenterProjected, lrint(fAreaWidth), lrint(fAreaHeight));
		    tVectorOfPoints::iterator it;
		    for(it=tVectorOfPointsSource.begin();it!=tVectorOfPointsSource.end();it++){
		    ...
		    tfPoint PointSource;
		    PointSource.first = fXCenterProjected;
		    PointSource.second = fYCenterProjected;
		    
		    tfPoint PointTarget;
		    PointTarget.first = iXTarget;
		    PointTarget.second =iYTarget;
...
//	    cout << "target larger than source" << endl;
	    ...
	    tVectorOfPoints tVectorOfPointsSource = clsBaseTopologySource->getPointsForSubPopulation(clsBaseSubPopulationSource);
//	    cout << "getting list for source" << endl;

	    tVectorOfPoints::iterator itLOPSource;
	    int iXStart = (*tVectorOfPointsSource.begin()).first;
	    int iYStart = (*tVectorOfPointsSource.begin()).second;
	    for(itLOPSource=tVectorOfPointsSource.begin();itLOPSource!=tVectorOfPointsSource.end();itLOPSource++){
...
//		cout << "(" << iXSource << "," << iYSource << "), ";
		...
		if(bAll){
		    tVectorOfPoints tVectorOfPointsTarget;
		    tVectorOfPointsTarget = clsBaseTopologyTarget->getPoints4Rect( fXCenterProjected, fYCenterProjected, lrint(fAreaWidth), lrint(fAreaHeight));
		    tVectorOfPoints::iterator it;
		    for(it=tVectorOfPointsTarget.begin();it!=tVectorOfPointsTarget.end();it++){
...
/*
  ...
  cout << ")" << endl;
*/
	expandXXX(listPointTriples, tVectorOfPointsSource, tVectorOfPointsTarget);
    }

    else if(dynamic_cast<ClsPatternTuples*>(clsPattern)){
//	cout << "ClsPatternTuples" << endl;
	if(clsPattern->getTuples()!=NULL){
	    list<pair<tVectorOfPoints, tVectorOfPoints> > lstPLP = clsPattern->getTuples()->getData();
//	    cout << "===========" << endl;
//	printTupleList(lstPLP);
	    
	    listIndexQuadruples.clear();
	    list<pair<tVectorOfPoints, tVectorOfPoints> >::iterator it;
	    for(it=lstPLP.begin(); it!=lstPLP.end(); it++){
		VectorPointTriples listPointTriplesLocal;
		expandXXX(listPointTriplesLocal, (*it).first, (*it).second);
//???		listPointTriples.merge(listPointTriplesLocal);

		merge(listPointTriples.begin(), listPointTriples.end(), listPointTriplesLocal.begin(), listPointTriplesLocal.end(), listPointTriples.begin());
...
ListIndexQuadruples ClsFEConnection::calculateArborization(const VectorPointTriples& listPointTriples, ClsBaseTopology *clsBaseTopologySource, ClsBaseTopology *clsBaseTopologyTarget ){

    cout << "ClsFEConnection::calculateArborization(const VectorPointTriples& listPointTriples, ClsBaseTopology *clsBaseTopologySource, ClsBaseTopology *clsBaseTopologyTarget)" << endl;
//    ClsBaseArborization *clsArborization;
	...
	cout << "Arb is PF" << endl;
	VectorPointTriples::const_iterator it;
	...
	cout << "Arb is RF" << endl;
//ORIG	int ii = 0;
//ORIG	VectorPointTriples::const_iterator it;
//ORIG	for(it=listPointTriples.begin();it!=listPointTriples.end();it++){
//ORIG	    double fXCenterSource = (*it).first.first; /* second is target */
//ORIG	    double fXCenterTarget = (*it).second.first; /* second is target */
//ORIG	    double fYCenterSource = (*it).first.second;
//ORIG	    double fYCenterTarget = (*it).second.second;
//ORIG
//ORIG//	    cout << "\t\tfXCenterSource fYCenterSource: " <<  fXCenterSource << ", " << fYCenterSource << endl;
//ORIG//	    cout << "\t\tfXCenterTarget fYCenterTarget: " <<  fXCenterTarget << ", " << fYCenterTarget << endl;
//ORIG
//ORIG	    int iIndexTarget = clsBaseTopologyTarget->pos2index(lrint(fXCenterTarget), lrint(fYCenterTarget)); /* CAST?? OR LRINT?? */
//ORIG	    list<tIndexDist> listIndexDist = clsBaseTopologySource->getCellsForArborization(fXCenterSource, fYCenterSource, clsArborization);
//ORIG	    list<tIndexDist>::iterator it1;
//ORIG	    for(it1=listIndexDist.begin(); it1!=listIndexDist.end(); it1++){
//ORIG		IndexQuadruple indexQuadruple((*it1).first, iIndexTarget, (*it1).second, 0);
//ORIG		listIndexQuadruplesTemp.push_back(indexQuadruple);
//ORIG	    }
//ORIG	    ii++;
//ORIG	    cout << "ii: " << ii << endl;
//ORIG
...
//total  33315984
	int ii = 0;
	int iVSize = listPointTriples.size();
	for(ii=0;ii<iVSize;ii++){
	    PointTriple pt = listPointTriples.back(); 
	    listPointTriples.pop_back(); 

	    double fXCenterSource = pt.first.first; /* second is target */
	    double fXCenterTarget = pt.second.first; /* second is target */
	    double fYCenterSource = pt.first.second;
	    double fYCenterTarget = pt.second.second;
...
// VectorPointTriples ClsFEConnection::expandXXX(VectorPointTriples &vectorPointTriples, const tVectorOfPoints &tVectorOfPointsSource , const tVectorOfPoints &tVectorOfPointsTarget){
//     VectorPointTriples listPointTriples;
//     tVectorOfPoints::const_iterator itLOPSource;
//     for(itLOPSource=tVectorOfPointsSource.begin();itLOPSource!=tVectorOfPointsSource.end();itLOPSource++){
// 	tVectorOfPoints::const_iterator itLOPTarget;
// 	for(itLOPTarget=tVectorOfPointsTarget.begin();itLOPTarget!=tVectorOfPointsTarget.end();itLOPTarget++){
...
void ClsFEConnection::expandXXX(VectorPointTriples &vectorPointTriples, const tVectorOfPoints &tVectorOfPointsSource , const tVectorOfPoints &tVectorOfPointsTarget){


//     typedef vector<tiPoint > tVectorOfPairs;
//     tVectorOfPairs tVectorOfPairsSource(tVectorOfPointsSource.size());
//     tVectorOfPairs tVectorOfPairsTarget(tVectorOfPointsTarget.size());

//     int ii=0;
//     tVectorOfPoints::const_iterator itLOPSource;
//     for(itLOPSource=tVectorOfPointsSource.begin();itLOPSource!=tVectorOfPointsSource.end();itLOPSource++){
// 	tVectorOfPairsSource[ii] = *itLOPSource;
// 	ii++;
//     }
//     ii=0;
//     tVectorOfPoints::const_iterator itLOPTarget;
//     for(itLOPTarget=tVectorOfPointsTarget.begin();itLOPTarget!=tVectorOfPointsTarget.end();itLOPTarget++){
// 	tVectorOfPairsTarget[ii] = *itLOPTarget;
// 	ii++;
//     }

    cout << "before inst" << endl;
    typedef vector<PointTriple> VectorPointTriples;
    vectorPointTriples.resize(tVectorOfPointsSource.size() * tVectorOfPointsTarget.size());
    cout << "after inst" << endl;



    int ii = 0;
    for(int aa = 0; aa < tVectorOfPointsSource.size(); aa++){
	tVectorOfPoints::const_iterator itLOPTarget;
	for(int bb = 0; bb < tVectorOfPointsTarget.size(); bb++){
	    PointTriple pointTriple((tVectorOfPointsSource[aa]), (tVectorOfPointsTarget[bb]), 0);
	    vectorPointTriples[ii] = pointTriple;
	    ii++;
	}
	cout << "aa: " << aa << endl;
    }
    cerr << "exit: ClsFEConnection::expandXXX" << endl;


//    return vectorPointTriples;
}
...
ListIndexQuadruples ClsFEConnection::PointTriples2IndexQuadruples(const VectorPointTriples& listPointTriples, ClsBaseTopology *clsBaseTopologySource, ClsBaseTopology *clsBaseTopologyTarget ){
...
*/
	...
	list<pair<tVectorOfPoints, tVectorOfPoints> > lstValid;

	list<pair<tVectorOfPoints, tVectorOfPoints> > lst = clsTuples->getData();
	list<pair<tVectorOfPoints, tVectorOfPoints> >::iterator itTop;
	for(itTop=lst.begin(); itTop!=lst.end(); itTop++){
	    tVectorOfPoints::iterator itPre;
	    tVectorOfPoints lstPre = (*itTop).first;
	    tVectorOfPoints::iterator itPost;
	    tVectorOfPoints lstPost = (*itTop).second;
	    
	    tVectorOfPoints lstPreValid;
	    tVectorOfPoints lstPostValid;
...
//		cout << "post x: " << itPost->first << ", y: " << itPost->second << endl;
		...
		pair<tVectorOfPoints, tVectorOfPoints> pairTemp(lstPreValid, lstPostValid);
...
/* ?? to do: remove invalid points */
	...
	vector<tiPoint> lstPoints = clsList->getData();

	vector<tiPoint> lstValidPoints;
