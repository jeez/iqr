 ...
 ** $Id: ClsFESystemManager.cpp,v 1.3 2003/08/12 16:04:15 ulysses Exp ulysses $
 **
 ** Copyright (C) 2002 by Ulysses Bernardet
 ** $Author: Ulysses Bernardet
 **
 ** $CreateDate: Tue Feb 19 18:06:06 2002
 ** $Date: 2003/08/12 16:04:15 $
 **
 ** $Log: ClsFESystemManager.cpp,v $
 ** Revision 1.3  2003/08/12 16:04:15  ulysses
 ** *** empty log message ***
 **
 ** Revision 1.2  2003/04/04 17:29:19  ulysses
 ** before removal of the ClsInfoGroup and ClsInfoPrc data structure usage
 **
 ** Revision 1.1  2003/04/03 17:04:26  ulysses
 ** Initial revision
 **
 ** Revision 1.2  2002/12/08 17:34:09  ulysses
 ** After separating system building, managing and serializing
 **
 ** Revision 1.1  2002/02/20 13:52:17  ulysses
 ** Initial revision
 **
 **
 ** $Keywords:
...
//***    clsInfoSystem.setDate( strDate );                 //***
//***    clsInfoSystem.setSystemName( "new system" );      //***
//***    clsInfoSystem.setHostname( getenv("HOSTNAME") );  //***
//***    clsInfoSystem.setAuthor( getenv("USER"));	      //***
//***    clsInfoSystem.setPort( 0) ;                       //***
...
/* things to be done here:
   - clear lstPrcs
   - create DOM tree for the system
   - create clear text XML representation of the system
   - fill lstPrcs

   for now (20020227), and in SimControl, there is no real serialization taking place! Rather
   port numbers for the FE and the Prcs are generated, and with the ClsFESerializer class the portnumbers in
   the DOMDocument (of the ClsSysFileParser) are changed. ClsFESerializer then return the system file as a std::string
*/
...
//    int iReturn = 0;
//     int iPortFE = ((double)rand() / (double)RAND_MAX)*500000 ;
...
//***    clsInfoSystem.setHostname( strHostname ); //***
//***    clsInfoSystem.setPort( iPortFE) ; //***
...
void ClsFESystemManager::deleteProcess(string strProcessID) {
	...
	for (itmapFEGroups = mapFEGroups.begin(); itmapFEGroups != mapFEGroups.end(); ++itmapFEGroups){
		...
		*/
		...
		emit sigItemDeleted(ClsFESystemManager::ITEM_GROUP, strGroupID); /* trouble maker....*/
	    }
	}
