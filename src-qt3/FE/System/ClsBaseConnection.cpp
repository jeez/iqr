 ...
 ** $Id: ClsBaseConnection.cpp,v 1.1 2003/08/19 13:33:20 ulysses Exp ulysses $
 **
 ** Copyright (C) 2003 by Ulysses Bernardet
 ** $Author: Ulysses Bernardet
 **
 ** $CreateDate: Mon Aug  4 17:52:45 2003
 ** $Date: 2003/08/19 13:33:20 $
 **
 ** $Log: ClsBaseConnection.cpp,v $
 ** Revision 1.1  2003/08/19 13:33:20  ulysses
 ** Initial revision
 **
 **
 ** $Keywords:
...
void ClsBaseConnection::printTupleList(list<pair<tListOfPoints, tListOfPoints> > lstPLP) {
    list<pair<tListOfPoints, tListOfPoints> >::iterator it0;
    for(it0=lstPLP.begin(); it0!=lstPLP.end(); it0++){

	tListOfPoints tListOfPointsSource = (*it0).first;
	tListOfPoints tListOfPointsTarget = (*it0).second;

//	    cout << "--------------" << endl;
	cout << "(";
	tListOfPoints::iterator itLOPSource;
	for(itLOPSource=tListOfPointsSource.begin();itLOPSource!=tListOfPointsSource.end();itLOPSource++){
...
//		int iIndexSource = clsBaseTopologySource->pos2index(iXSource, iYSource);
	...
	cout << ")<->(";
	tListOfPoints::iterator itLOPTarget;

	for(itLOPTarget=tListOfPointsTarget.begin();itLOPTarget!=tListOfPointsTarget.end();itLOPTarget++){
