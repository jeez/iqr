      ...
      bStretchable(_bStretchable)
{
	...
	cout << "ClsParameterWidget::ClsParameterWidget: created widget "
	     << _pcName
	     << endl;
    }
    ...
    pqpalChanged = new QPalette();
    // Get default widget palette from application.
    *pqpalChanged = qApp->palette().copy();
    // Labels draw text in the foreground color.
    pqpalChanged->setColor(QColorGroup::Foreground, Qt::blue);
    // Value widgets draw text in the text color.
    pqpalChanged->setColor(QColorGroup::Text, Qt::red);
}
 ...
 */
void 
iqrfe::ClsParameterWidget::connectWidget(const ParameterWidgetMap &_mapWidgets)
{
 ...
 */
void 
iqrfe::ClsParameterWidget::apply(ClsParameter *_pParameter)
{
    if (_pParameter != 0) {
 ...
 */
void 
iqrfe::ClsParameterWidget::showChanged(bool _bChanged)
{
    if (_bChanged) {
 ...
 */
void 
iqrfe::ClsParameterWidget::connectToEnabler(const ParameterWidgetMap &_mapWidgets)
{
    ...
    ClsParameterWidget *pEnablerWidget = findWidget(_mapWidgets, 
						    getEnabler());
    if (pEnablerWidget != 0) {
...
iqrfe::ClsParameterWidget::findWidget(const ParameterWidgetMap &_mapWidgets, 
