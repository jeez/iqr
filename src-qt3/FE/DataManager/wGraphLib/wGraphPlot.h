...
public:
  ...
  WGraphTrace *getTrace(int trace) { return traces.at(trace); }

  /*! Returns a reference to the data of trace \em trace.*/
  QMemArray<float> &traceData(int trace) { 
    return traces.at(trace)->getData(); 
  };
  ...
  QMemArray<float> &getXData() { return xData; }
...
protected:
  ...
  QMemArray<float>  xData;

  QPtrList<WGraphTrace> traces;
