...
inline QTextStream & operator<<(QTextStream &s, const WRange &r) {
  s<<r.min()<<" "<<r.max()<<" ";
  return s;
};

inline QTextStream & operator>>(QTextStream &s, WRange &r){
  float min,max;
  s>>min>>max;
  r=WRange(min,max);
  return s;
...
inline QDataStream & operator<<(QDataStream &s, const bool &b){
  return s.writeRawBytes((const char*)&b,sizeof(bool));
};

inline QDataStream & operator>>(QDataStream &s, bool &b) {
  return s.readRawBytes((char *)&b,sizeof(bool));
}
