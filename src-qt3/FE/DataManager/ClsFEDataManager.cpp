 ...
 ** $Header: /home/ulysses/Code/iqr421_Redesign/FE/DataManager/ClsFEDataManager.cpp,v 1.7 2004/02/02 16:48:03 ulysses Exp ulysses $
 **
...
void ClsFEDataManager::customEvent(QCustomEvent *e){
#ifdef DEBUG_CLSFEDATAMANAGER
    cout << "ClsFEDataManager::customEvent(QCustomEvent *e)" << endl;
#endif
...
void ClsFEDataManager::updateClients(){
    ...
    /* NEW 2004/04/12: mutex locked in the DataClients (should be more efficient...
    qmutexSysGUI->lock();
    */
    map<string, ClsFEBasePlot*>::iterator mapIteratorPlots;
    for(mapIteratorPlots = mapPlots.begin(); mapIteratorPlots!=mapPlots.end(); ++mapIteratorPlots){
...
//    sleep(2);
/* NEW 2004/04/12: mutex locked in the DataClients   
   qmutexSysGUI->unlock(); 
*/
...
#endif
    map<string, ClsFEBasePlot*>::iterator itPlots;
    for(itPlots = mapPlots.begin(); itPlots != mapPlots.end(); itPlots++){
	if(itPlots->second!=NULL){
	    itPlots->second->close();
	}
    }
