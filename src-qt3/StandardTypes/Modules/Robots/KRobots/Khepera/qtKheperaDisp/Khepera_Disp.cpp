...
//	qtarch_btnStart->setPalettePropagation( QWidget::NoChildren );
	...
	connect(qtarch_btnStart, SIGNAL(clicked()), SLOT(DoKhepera()) );
...
//	connect(clsKhepDisp, SIGNAL(GraphClicked( int )), this, SLOT(GraphClicked( int )) );
...
void qtProtekData::DoKhepera() {
//     clsKhepDisp->DrawStatus(strType, iTime, fValue);     
     ...
     clsKhepDisp->setCollision( aiColl );
     clsKhepDisp->setIR( afIR );
     clsKhepDisp->setMotor( afMotor );
     clsKhepDisp->setInhibition( iInhibit );
     clsKhepDisp->DrawStatus( );


     QString qstrFileName = "/disks/zebra3/ulysses/Code/Khepera_Disp/test1.ps";

     clsKhepDisp->setFileName ( qstrFileName );
     clsKhepDisp->saveGraph();
...
//		   printf("Motor: %i\n", qstrT.toInt());
	      ...
	      iInhibit = qstrInhibition.toInt();
  	      ...
  	      ClsKhepDisp clsKhepDisp("Khepera Display");
